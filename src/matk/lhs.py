...
def lhsFromDensity(kde, siz=100):
    ...
    :Parameters:
        ...
        - `siz`: Number or shape tuple for the output sample
    """
    if not isinstance(kde, stats.kde.gaussian_kde):
...
def lhs(dist, parms, siz=100, noCorrRestr=False, corrmat=None, seed=None):
    ...
    dist is a scipy.stats random number generator
    ...
    indices = rank_restr(
        nvars=len(dists), smp=siz, noCorrRestr=noCorrRestr, Corrmat=corrmat)
    smplist = []
...
def rank_restr(nvars=4, smp=100, noCorrRestr=False, Corrmat=None):
    ...
    def shuf(s):
        """
        Shuffle a vector, making sure to make a copy of the original
    ...
    if noCorrRestr or nvars == 1:
        ...
        if Corrmat is None:
            ...
            if Corrmat.shape[0] != nvars:
                raise TypeError('Correlation matrix must be of rank {}'.format(nvars))
        ...
        x = [stats.rankdata(Final.transpose()[i, ]) for i in range(nvars)]
...
if __name__=='__main__':
    ...
    # plot(numpy.arange(min(min(c), min(n)), max(max(c), max(n)), .1),
    #      dist(*pars).pdf(numpy.arange(min(min(c), min(n)), max(max(c), max(n)), .1)),
    #      label='PDF')
    #legend()
